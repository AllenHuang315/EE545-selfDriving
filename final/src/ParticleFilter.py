...
class ParticleFilter():
  ...
  def expected_pose(self):
    # YOUR CODE HERE
    expect_x = np.sum(self.particles[:, 0] * self.weights)
    expect_y = np.sum(self.particles[:, 1] * self.weights)
    sin_theta_mean = np.mean(np.sin(self.particles[:, 2]) * self.weights)
    cos_theta_mean = np.mean(np.cos(self.particles[:, 2]) * self.weights)
    expect_theta = np.arctan2(sin_theta_mean, cos_theta_mean)

    return np.array([expect_x, expect_y, expect_theta])
