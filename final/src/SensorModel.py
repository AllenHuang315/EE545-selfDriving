...
SIGMA_HIT = 1.5# Noise value for hit reading
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    for d in range(table_width):
      ...
      for r in range(table_width): 
        ...
        ztk, ztk_star = np.float32(r), np.float32(d)

        # Gaussian Distribution - Correct range with local measurement noise
        if 0.0 <= ztk <= max_range_px:
          # ignore the normalizer n, we will normalize at the end
          Phit = (1.0 / np.sqrt((2.0 * np.pi * (SIGMA_HIT**2)))) * np.exp(-0.5 * (ztk - ztk_star)**2 / SIGMA_HIT**2)
          row_sum += Phit
        else:
          Phit = 0.0

        # Exponential distribution - Unexpected Objects
        if 0.0 <= ztk < ztk_star:
          normlizer_n = 1.0 / (1.0 - np.exp(-lambda_short * ztk_star))
          Pshort = normlizer_n * lambda_short * np.exp(-lambda_short * ztk)
        else:
          Pshort = 0.0
        
        # Uniform distribution - Failures 
        if ztk == max_range_px:
          Pmax = 1.0
        else:
          Pmax = 0.0

        # Uniform distribution - Random measurements
        if 0.0 <= ztk < max_range_px:
          Prand = 1.0 / max_range_px
        else:
          Prand = 0.0
        intrinsic_params = np.array([Z_HIT, Z_SHORT, Z_MAX, Z_RAND])
        prob = np.array([Phit, Pshort, Pmax, Prand])
        sensor_model_table[r, d] = np.dot(intrinsic_params, prob)
      # Normaliz 
      sensor_model_table[d] /= row_sum
