...
class KinematicMotionModel:
  ...
  def motion_cb(self, msg):
    ...
    dt = dt.to_sec()
    # YOUR CODE HERE
    particle_len = self.particles.size / 3
    speed_noise = np.random.normal(curr_speed, KM_V_NOISE, particle_len)
    angle_noise = np.random.normal(curr_steer_angle, KM_DELTA_NOISE, particle_len)
    
    sin_2B = np.sin(2 * (np.arctan(0.5 * np.tan(angle_noise))))
    theta_T_1 = self.particles[:,2].copy()

    self.particles[:,2] += (speed_noise / self.CAR_LENGTH) * sin_2B * dt
    self.particles[:,2] = np.mod(self.particles[:,2] + np.pi, 2 * np.pi) - np.pi
    self.particles[:,0] += (self.CAR_LENGTH / sin_2B) * (np.sin(self.particles[:,2]) - np.sin(theta_T_1))
    self.particles[:,1] += (self.CAR_LENGTH / sin_2B) * (-np.cos(self.particles[:,2]) + np.cos(theta_T_1))

    self.particles[:,2] += np.random.normal(0, KM_THETA_FIX_NOISE, particle_len)
    self.particles[:,1] += np.random.normal(0, KM_Y_FIX_NOISE, particle_len)
    self.particles[:,0] += np.random.normal(0, KM_X_FIX_NOISE, particle_len)
...
TEST_STEERING_ANGLE = -0.222 # radians
TEST_DT = 1.5 # seconds
